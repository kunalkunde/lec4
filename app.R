...
ui <- fluidPage(
    titlePanel("360 ONE Plus: Holding Period IRR for a bond"),
    # Bond mty; Buy date, yield; Sell date, yield; Coupon rate, frequency
    fluidRow(
      column(4, dateInput(inputId = "dateOfMaturity", label = h4("Bond maturity"), value = "2032-01-17", format = "yyyy-mm-dd")),
      column(4, sliderInput(inputId = "cpnRate", label = h4("Coupon (% p.a.)"), min=0, max=10, value=6.54, step = 0.01)),
      column(4, radioButtons(inputId = "cpnPmtFreq",label = h4("Coupon frequency"),choices = c(1, 2, 4),selected = 1))
    ),
    fluidRow(
      column(4, dateInput(inputId = "dateOfPurchase", label = h4("Buy date"), value = "2016-06-16", format = "yyyy-mm-dd")),
      column(4, sliderInput(inputId = "buyYld", label = h4("Buy Yield (% p.a.)"), min=5, max=15, value=5.55, step = 0.01))
    ),
    fluidRow(
      column(4, dateInput(inputId = "dateOfSale", label = h4("Sell date"), value = "2027-08-16"), format = "yyyy-mm-dd"),
      column(4, sliderInput(inputId = "sellYld", label = h4("Sell Yield (% p.a.)"), min=5, max=15, value=7.85, step = 0.01))
    ),
